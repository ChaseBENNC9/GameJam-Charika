...
public class SecurityLightMovement : MonoBehaviour
{
    ...
    private const int rangeValue = 20; //the value for the range

    // Start is called before the first frame update
    void Start()
    {
        range = new Vector3(rangeValue, lightHeight, rangeValue); //set the range
        targetPos = new Vector3(0, lightHeight, 0); //initial target so the light doesn't fall through floor
    }
    ...
    void Update()
    {
        ...
        {
            targetPos = new Vector3(Random.Range(-range.x, range.x), lightHeight, Random.Range(-range.z, range.z)); //set a new target
